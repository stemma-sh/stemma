...
- Status: **implemented** (v1: sections 1–4 + `render`, 2026-07-04; intent
  manifests and the disposition accept/reject split are v1.1 — see
...
1. **Session review** (the primary workflow): a document handle retains its
   ...
   *everything this session changed* — a structured census, a proof that
   everything else is untouched, and optionally the change set rendered as a
   redline docx. Saving is the commit gate: review, then save if it looks
   right.
2. **Stateless audit** (the same core, externalized): `audit(before, after)`
   ...
   isn't stemma (another tool, a human, a raw-XML agent). This is the
...
- **The domain already has the right UX for a diff.** `compare_docx`
  (`diff_documents`) renders base→target as a tracked-changes document —
  the redline IS the native, universally-understood diff visualization.
...
is persistent/immutable and returns a new document:
...
near-free), and (b) `review()` computes the `AuditReport` between baseline
...
1. **Revision census delta** — tracked changes present in `after` but not in
   `before`, attributed by the same watermark discipline the write receipts
   ...
   content comparison against the baseline census — accept-vs-reject
...
2. **Untracked (direct) delta** — blocks whose committed content differs
...
3. **Untouched proof** — every block outside sections 1–2 is verified
...
4. **Package verdict** — the existing validator report on `after`.
5. **Intent check (optional)** — caller-supplied manifest
   (`expected: [{kind, block_hint?, must_contain?, must_not_contain?}]`)
   diffed against sections 1–2: expectations with no matching change, and
...
Plus an optional **rendered redline**: `render: {path}` materializes
sections 1–2 as a tracked-changes docx via the existing `diff_documents`
...
```jsonc
// Session review — no new state model, the handle already exists:
...
- **Commit-gate that refuses to save on a dirty audit.** Rejected for v1:
  ...
  be exactly what the user wants to ship). The gate is doctrine (SKILL:
  review → save), not mechanism. Revisit after usage data.
- **Byte diff of the packages.** Rejected: churn-vs-change is precisely the
  problem the canonical IR exists to solve.

## Rollout

1. **v1 (engine + MCP)**: baseline retention; `AuditReport` sections 1–4;
...
2. **v1.1**: intent manifests (section 5); SKILL doctrine update
   ("review before save" as the standing rule — a disclosed suite version
...
4. **Bindings**: `audit`/`review` are the first verbs exposed if/when
   Python bindings land — they are the library market's front door.

## Open questions

- Naming: `review_session` vs `diff_session` vs `pending`. "Diff" undersells
  sections 3–5; "audit" may read heavy for the in-session form.
...
- Should `save_docx` grow an optional `require: {validator_ok: true,
  untouched_ok: true}` hard gate? (Deferred; see Alternatives.)
...
- Whether `disposition` attribution for pre-existing revisions belongs in
  v1 or v1.1 — it is the most subtle part (it is exactly the accept/reject
  ...
  it coincides with — attributed, never silently dropped.)*
...
`render`, and — as a prerequisite — completion of the revision enumeration
...
- **Census attribution is by record identity, never id ranges.** Export
  ...
  which is also what makes the disposition judgment content-based rather
