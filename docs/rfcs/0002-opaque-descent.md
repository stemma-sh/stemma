# RFC 0002: opaque descent — surgical edits inside textboxes and content controls

- Status: **implemented** (v1 phases 0–3b, 2026-07-08; hardened by the
...
deliberate — fully modeling textbox/SDT interiors (styles, geometry,
anchoring) is a fidelity trap — but it left a capability gap: **no verb could
...
1. **The opaque boundary stays.** Descent is a *scoped edit capability*, not
   ...
   that produced the imported bytes — not a byte-for-byte guarantee on
...
- **Phase 0 — shared walk + discovery.** `visit_opaque_interiors` is the one
  ...
  same walk.
- **Phase 1 — fragment tracked-splice core + `opaque_text_edit`.** An
  ...
  (`w:hyperlink`/`w:smartTag` — Word edits their text freely) interleaved as
...
- **Phase 2 — `sdt_text_fill` + block-SDT plumbing.** The forms-natural
  ...
  unreachable from the pure edit core — the fill is validated and id-minted
...
serialize/reload — a minted `w:ins` becomes byte-identical to a pre-existing
one — so it is not a model-honest invariant. The shipped rule is a property
of the markup and of the id population:
...
- **`w:ins`/`w:del` only** — a move is a pair (`w:moveFrom`+`w:moveTo` plus
  ...
  them pair-correctly.
- **unique document-wide** — import normalizes body/story revision ids but
  deliberately never rewrites opaque bytes, so interior carriers keep raw
...
Everything else — stacked, `*PrChange`, move, id-less, id-0, unparseable,
duplicate-id — is censused as `OpaqueInterior` with the never-selectable
sentinel id 0.
...
— a documented v1 limitation; bulk accept/reject keeps copies consistent).
...
controls); the one measured over-refusal — text inside hyperlinks/smart tags
(1.8%) — was closed by the wrapper descent in Phase 1, bringing targeted
...
- duplicate-wild-id demotion (the uniqueness clause above) — previously a
...
- minted-id floor over block-opaque interior ids (invisible to the pure
  core's scan);
- `semantic_hash` on a block fill refuses (cannot be honored — block
  discovery surfaces no hash) instead of being silently dropped; duplicate
...
engine): every witness passes presence parity per story family — wherever
Word reports tracked revisions in a story family, our per-story enumeration
